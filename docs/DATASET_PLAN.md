...
| `bestTimeToVisit` (ProtectedArea only, not Zoo, added 2026-08-04) | `string \| null` | Best season to visit, e.g. "November to April is best...". Zoos are ex-situ, open year-round regardless of season, so this field doesn't apply there. Derived (no new source/network call) from the entity's own name + `uniqueFeatures` text first (park-specific, most reliable), falling back to its flagship species' habitat description only when neither matches — see `2026-08-04-generate-best-time-to-visit.py` for the habitat-keyword buckets (alpine/desert/marine/Western Ghats/bird-migratory/default) and the Himalayan-state gate that keeps South Indian Western Ghats "high-altitude" shola parks from being misclassified as Himalayan-snowbound. |
